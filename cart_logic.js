...
function calculateSubtotal (prodTotals) {
    ...
    var gross_total = 0.00;
    for (i=0; i<3; i++) {
        gross_total = gross_total + prodTotals[i];
    }

    return gross_total;   // replace this!
}
...
function updateProdAvailability(product, selected, availability) {
    ...
    var messageText = "in stock";
    if (selected > availability) {
        availText = "OUT_OF_STOCK";
        messageText = product + "out of stock";
    } else if (selected >= (availability - 2)){
        availText = "LIMITED_SUPPLY";
        messageText = "limited quantities";
    }

    updateProdAvailabilityLabel(product, messageText, availText);
